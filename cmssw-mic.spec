### RPM cms cmssw-mic CMSSW_6_2_0_pre8
...
%define saveDeps        yes
%define source1         %{cvsrepo}&tag=-r%{realversion}&module=Utilities/Timing&export=%{srctree}&output=/src.tar.gz
%define patchsrc4       perl -p -i -e 's!(<classpath.*/tests\\+.*>)!!;' config/BuildFile.xml; \
                        ...
                        sed -i -e 's|^CMD_python := $(PYTHON_BASE)/bin/python|CMD_python := true|' config/SCRAM/GMake/Makefile.rules; mkdir src/Utilities; mv src/Timing src/Utilities
